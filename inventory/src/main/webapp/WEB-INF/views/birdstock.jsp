...
<div class="container">
	<table class="table" id="tblStockForUpdate">
		<thead class="">
			...
			<c:forEach var="bstockobj" items="${birdStock}">
				<tr>
					<td>${bstockobj.birdName}</td>
					<td>${bstockobj.batchNo}</td>
					<td>${bstockobj.rate }</td>
					<td>${bstockobj.quantity}</td>
					<td><input data-popup-open="popup-2" type="button" value="Update"/></td>
					<td><input type="hidden" value="${bstockobj.recordID}"/></td>
				</tr>
			</c:forEach>
		</thead>
	</table>
