	...
	<table class="table" id="tblStockForUpdate">
		<thead class="">
			...
			<c:forEach var="fstockobj" items="${fishStock}">
				<tr>
					<td>${fstockobj.fishCategory}</td>
					<td>${fstockobj.batchNo}</td>
					<td>${fstockobj.rate }</td>
					<td>${fstockobj.quantity}</td>
					<td><input data-popup-open="popup-2" type="button" class="formBtn btn btn-primary" value="Update"/></td>
					<td><input type="hidden" value="${fstockobj.recordID}"/></td>
				</tr>
			</c:forEach>
		</thead>
	</table>
		...
		<div class="popup-inner" style="height:350px !important;">
			...
			<div id="frmupdatedetails" class="container" style="position: absolute;top:20%;margin-top:10px;">
			  <form action="${pageContext.request.contextPath}/updateDetails" method="post">
			    ...
			    <div class="row margin-bottom-inputs">
				    <div class="col-md-3"><input name="categorySubmit" class="formBtn btn btn-secondary" type="submit" value="Update"/></div>
